...
var client      = modbus.createTCPClient(8000, '127.0.0.1'),
    cntr        = 0,
...
client.readInputRegister (0, 8, function (resp) { 
    console.log('inside the first user cb');
    console.log(resp);
    closeClient(); 
});

client.readInputRegister (6, 10, function (resp) { 
    console.log('inside the second user cb');
    console.log(resp);
    closeClient(); 
});

client.readCoils (0, 2, function (resp, err) { 
